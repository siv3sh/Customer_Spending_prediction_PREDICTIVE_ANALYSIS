"""
Standalone script to generate comprehensive DOCX report
Run this script to create a DOCX file in the current folder
"""
import pandas as pd
import numpy as np
...
def create_graph_explanations_section(doc, df, df_processed, models, results, best_model_name, X_train_scaled, scaler, label_encoders):
    """Add detailed graph explanations and analysis"""
...
The distribution analysis shows that spending scores are relatively evenly distributed across the range of 1-100, suggesting diverse customer spending patterns. This diversity is valuable for segmentation and targeted marketing strategies."""
    ...
    corr_analysis = f"""The correlation matrix visualization reveals the strength and direction of relationships between variables:
...
The correlation heatmap visualization helps identify which features have the strongest relationships with spending scores, guiding feature selection and model building."""
...
• Visual Pattern: The scatter plot shows the distribution of spending scores across different age groups, allowing identification of clusters and outliers.
...
The box plots by age groups show the distribution, quartiles, and outliers for each age category, providing insights into spending variability within each group."""
...
• Income Distribution: The scatter plot reveals how spending scores vary across different income levels.
...
The regression line shows the trend, while box plots reveal the distribution and identify potential outliers or unusual patterns within each income group."""
...
The pie chart shows the gender distribution, while box plots and violin plots reveal the distribution shape, spread, and potential differences in spending behavior between genders. The violin plots are particularly useful as they combine box plot information with kernel density estimation, showing the probability density of spending scores at different values."""
...
• Multi-dimensional Insight: Unlike 2D plots that show relationships between two variables, the 3D plot reveals how all three variables interact together.
• Color Coding: The spending score is color-coded, making it easy to identify patterns and clusters of customers with similar characteristics.
...
This comprehensive view is crucial for understanding complex customer behavior patterns and identifying segments for targeted marketing."""
...
def generate_comprehensive_docx():
    """Generate comprehensive DOCX report with all analysis"""
    ...
    # Import the main report content function
    from docx_generator import create_docx_report
    import io
    
    # Create the base report
    print("Creating base report structure...")
    temp_buffer = create_docx_report(df, df_processed, models, results, best_model_name, 
                                     X_train_scaled, scaler, label_encoders)
    
    # Read the temporary docx
    temp_doc = Document(io.BytesIO(temp_buffer.getvalue()))
    
    # Copy all elements to new document
    for element in temp_doc.element.body:
        doc.element.body.append(element)
    
    # Add graph explanations section
    print("Adding graph explanations and analysis...")
    create_graph_explanations_section(doc, df, df_processed, models, results, best_model_name, 
                                     X_train_scaled, scaler, label_encoders)
...
if __name__ == "__main__":
    try:
        output_file = generate_comprehensive_docx()
